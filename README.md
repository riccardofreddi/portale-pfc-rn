...
|---|---|---|
| **v4.72** | 1.72.0 (js472) | Cassetto — RISOLTO DEFINITIVAMENTE l'anteprima che riapriva il vecchio file: la copia locale dell'anteprima riusava il vecchio contenuto quando il server riusa la stessa chiave per lo stesso tipo/anno (es. iban_2026.pdf cancellato e ricaricato); ora la cache porta la versione del file (lastModified) nel nome e l'anteprima è sempre fresca (vale per tutto il Cassetto e l'Archivio). L'IBAN scritto a mano NON sta più sul telefono: al "Salva" l'app carica un piccolo file di testo sul server con l'upload esistente (tipo IBAN → iban_&lt;anno&gt;.txt): una sola fonte di verità, lista/anteprima/elimina/rinomina identici a qualsiasi altro documento; "Elimina" cancella dal server e lo slot torna vuoto; migrazione automatica (una volta sola) dell'IBAN scritto con la v4.71; nel modal di Aggiungi con tipo IBAN selezionato compare "Oppure scrivi l'IBAN a mano" |
| **v4.56** | 1.43.0 (js456) | Archivio: la targhetta della scadenza diventa "⏰ SCADENZA gg/mm" (prima "⏰ SCADE gg/mm") — solo l'etichetta, più chiara; nessun'altra modifica. "SCADE OGGI", "SCADUTA" e "✓ PAGATA" restano come erano |
| **v4.55** | 1.43.0 (js455) | Scadenze nell'Archivio: la riga del documento con una scadenza impostata dallo studio mostra la targhetta di stato ("⏰ SCADE gg/mm" ambra nel periodo di preavviso, neutra oltre; "⏰ SCADE OGGI"/"⏰ SCADUTA" rosso; "✓ PAGATA" verde) e una nuova azione rapida a spunta accanto a stellina e scarica: premuta (con conferma) segna la scadenza PAGATA e il server FERMA TUTTE le notifiche di quella scadenza (cron, banner web, campanella) registrando chi/quando; la spunta piena verde, ripremuta, riattiva le notifiche. Dopo il tocco la riga si aggiorna subito e i promemoria locali (allarme 08:30 del giorno di scadenza) vengono ri-regolati senza aspettare il ricaricamento. Richiede il server v4.60 |
| **v4.53** | 1.43.0 | Cassetto: regola "uno slot per tipo" + limite 5MB — ogni tipo (QR Code P.IVA, Certificato P.IVA, Visura Camerale, Doc. Identità, IBAN) si può caricare UNA volta sola: nella scelta del tipo, quelli già presenti restano visibili in grigio con la targhetta "già caricato" e il tocco spento; per ricaricare, prima si cancella il vecchio. La regola vale anche rinominando il file (il tipo resta inciso nella chiave lato server: rinominare non libera più lo slot) e per gli upload dal sito web (il server rifiuta con messaggio chiaro). Limite di dimensione del Cassetto portato da 20MB a 5MB (le altre superfici restano a 20MB). Risparmio di archivio garantito: massimo 5 file per cliente, lo spazio si libera cancellando |
| **v4.52** | 1.43.0 | Cassetto: risolto l'errore al caricamento ("stat path null") — il peso del file non blocca più l'upload: si legge solo per i veri file del telefono (file://) e solo per avvisare prima; i file scelti da "Recenti"/Download/cloud (indirizzi content:// che la lettura locale non sa aprire) salgono lo stesso e il limite vero lo controlla il server. Stesso funzionamento della risposta ai messaggi, che ha sempre funzionato |
| **v4.51** | 1.43.0 | Notifiche: auto-riparazione silenziosa — l'app si ri-presenta al server da sola all'avvio (quando la sessione ripristinata è valida) e a ogni ritorno in primo piano (max 1 volta ogni 10 minuti), aggiornando il token FCM senza MAI mostrare prompt o schermate (usa i permessi solo in lettura). Finora il token arrivava al server solo al login o all'interruttore Notifiche: se Google lo rinnovava nel frattempo (disinstallazione, pulizia, aggiornamento, a volte a distanza di mesi) le notifiche sparivano finché il cliente non rifaceva il login. Ora il server si aggiorna da solo alla riapertura dell'app. Rispettata la regola v4.13: interruttore Notifiche spento = nessuna ri-registrazione. Igiene: il listener del rinnovo token si attacca una sola volta per tutta la vita dell'app (prima ogni registrazione ne aggiungeva un altro) |
| **v4.50** | 1.43.0 | Messaggi: il "Nuovo" decide il titolare, non il rientro nella tab — la fotografia dei NUOVI non si azzera più uscendo e rientrando nella tab: la targhetta "Nuovo" e la pillola oro con il contatore restano a schermo finché l'utente non preme "Segna tutti letti" (prima bastava cambiare tab e tornare per spegnerle). "Quando rispondi togli nuovo": caricata la risposta, la targhetta "Nuovo" sparisce da quella scheda (vince il verde "Documento Inviato") ed esce dal contatore della pillola oro. Igiene della fotografia a ogni caricamento: escono chi è stato risposto o archiviato, così il contatore non conta mai messaggi già chiusi. Il pallino rosso del tab si spegne sempre all'ingresso, come da sempre |
| **v4.49** | 1.43.0 | Messaggi: intestazione piccola come in Cassetto e Registro attività — riquadro blu con gradiente, icona oro a bolla, scritta oro "MESSAGGI" e titolo "Posta riservata" (scelta del titolare, al posto di "Comunicazioni dallo Studio" che duplicava la striscia della home). Il messaggio NUOVO non è più scuro: il tondo dell'icona passa da navy pieno a tinta navy soft con lettera navy (distinzione nuovo/letto garantita da fondo tinto, titolo in grassetto e pillola rossa "Nuovo"). "Carica la risposta" e "Archivia/Ripristina" sono SEMPRE visibili sotto ogni messaggio, senza più dover toccare la scheda (il tocco apre/chiude solo il testo; tolta l'iconcina archivia in alto a destra, ormai doppione). Archivio: la scheda blu viene ricostruita con la stessa costruzione del Cassetto (disegno esatto senza sbordo, niente animazione sulla scheda, bordo invisibile dello stesso colore del fondo) per eliminare la riga nera attorno alla pillola "Documenti Nuovi"; verificata anche la variante senza documenti nuovi |
| **v4.48** | 1.43.0 | Archivio: addio definitivo alla "riga nera" — "I miei preferiti" esce dal riquadro blu e va appena sotto la barra di ricerca, con la STESSA grafica e grandezza della pillola "Documenti Nuovi" (oro pieno, dimensioni identiche, per scelta del titolare). Nell'hero blu resta al massimo un solo elemento oro, così la striscia scura fra i due pulsanti non può più formarsi, per costruzione. La pillola "Documenti Nuovi" resta invariata (compare solo quando ci sono davvero nuovi documenti); gradiente e lista anni intatti |
| **v4.47** | 1.43.0 | Archivio: grafica dell'hero riportata ESATTAMENTE a com'era (identica al Cassetto, v4.39) — annullati e rimossi su disposizione del titolare i tentativi v4.44-v4.46 (colori più chiari, pillola spostata, pannello-velo): la pillola "Documenti Nuovi" torna al posto di sempre, sopra "I miei preferiti", e la striscia scura fra i due elementi oro quando ci sono documenti nuovi resta come caratteristica accettata. Resta intatto il fix v4.43 (lista anni sempre aggiornata, anche a app viva) |
| **v4.43** | 1.43.0 | Archivio sempre aggiornato: la lista degli ANNI si ricarica a ogni sincronizzazione (prima partiva solo alla prima apertura). Se lo studio creava un anno nuovo (es. 2026 con i nuovi F24) mentre l'app era aperta in background, il chip dell'anno non compariva e i documenti sembravano spariti fino al riavvio totale dell'app. Ora il chip del nuovo anno appare da solo, senza riavvii |
| **v4.42** | 1.42.0 | "File inviato" leggibile: il server attacca al nome del file l'ID interno del messaggio ("scan.pdf (msg: cmd9xk…)"), un codice che al cliente non dice nulla. La pulizia taglia la parentesi col codice e lascia SOLO il nome del file — anche nello storico vecchio, perché avviene alla visualizzazione. Per disposizione del titolare: backend e portale web ZERO tocchi (il server continua a scrivere come sempre, è l'app a pulire) |
| **v4.41** | 1.41.0 | Registro attività che parla la lingua del cliente: mappa allineata ai veri codici del server (via le 5 pillole col codice grezzo tipo "RINOMINA_FILE"), diciture da 2 parole ("File scaricato", "File caricato", "File inviato", "Cartella scaricata"…), dettagli ridotti all'essenziale e puliti anche nello storico (solo il nome del file invece del percorso completo; rinomina = solo il nome nuovo; archivio = solo "N file"; "Accesso eseguito" e "Logout manuale" intatti), colori con una regola: verde = operazione completata, blu = solo lettura, grigio = organizzazione, rosso = attenzione. Server e logica intatti |
| **v4.40** | 1.40.0 | Messaggi: la data non tocca più le icone — esce dalla riga "STUDIO PFC • data" (che sugli schermi stretti sbordava sotto la freccia di espansione e l'icona archivio) e va su una sua riga sotto il titolo, con mesi brevi ("15 set 2026"); le due icone (apri/chiudi + archivia) stanno in una capsula morbida in alto a destra. Attività: via "& audit" e via la descrizione "consultazioni, download e accessi…", testata con la firma visiva dell'app (hero blu notte → blu primario → blu notte con bordo oro, identico a Cassetto e Archivio). Grafica sola, logica intatta |
...
| Caricamento nel Cassetto fallito ("stat path null") | Corretto dalla v4.52: il peso del file non blocca più il caricamento; il limite vero lo controlla il server (5MB dalla v4.53). Installa l'APK nuovo dalla release `latest-apk` |
| "Perché non posso caricare di nuovo lo stesso tipo?" | Dalla v4.53 ogni tipo si carica UNA volta sola (regola "uno slot per tipo"): nella scelta appare in grigio con "già caricato". Per ricaricarlo, cancella prima quello esistente — anche rinominare il vecchio non libera lo slot |
